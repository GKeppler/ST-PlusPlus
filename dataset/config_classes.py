...
from ALS4L.deep_learning_components.data_processing.augmentations.semantic_segmentation.augmentation_segmentation import (
    ImgSegProcessingPipeline,
    SemanticSegmentationProccesor,
)
from ALS4L.utils.configuration.mappings import (
    get_backbone,
    get_datamodule,
    get_labeler,
    get_learning_rate_scheudler,
    get_loss_function,
    get_model,
    get_optimizer,
    get_query_strategy,
...
def append_to_transforms(
    ...
    img_processing_pipeline = ImgSegProcessingPipeline(Namespace(**img_processing_args))
    if val_augs is not None:
        img_processing_pipeline.make_val_aug_transform(Namespace(**val_augs))

    train_transforms.append(
        SemanticSegmentationProccesor(img_processing_pipeline.get_train_transform())
    )
    val_transforms.append(
        SemanticSegmentationProccesor(img_processing_pipeline.get_val_transform())
    )
...
class ModelBuilder:
    def __init__(self, model_params: dict, checkpoint_path: str = None):
        model_args = split_parameters(dict_to_config(model_params), ["parameters"])[
            "parameters"
        ]
        ...
        if "backbone" in backbone_config:
            ...
            backbone_args = split_parameters(
                dict_to_config(backbone_config), ["parameters"]
            )["parameters"]
            if "checkpoint_prefix" in backbone_args:
                if (
                    backbone_args["checkpoint_prefix"] is not None
                    and backbone_args["checkpoint_path"] is not None
                    and backbone_args["checkpoint_path"].lower() != "imagenet"
                ):
            ...
            if "checkpoint_path" in backbone_args:
                if backbone_args["checkpoint_path"] is not None:
                    logging.info(
                        f"Loading pre-trained model with weights: {backbone_args['checkpoint_path']}"
                    )

            model_args["backbone"] = get_backbone(backbone_config["name"])(
                **backbone_args
            )
        ...
        if checkpoint_path == None:
            ...
            self.instance = model_type.load_from_checkpoint(
        ...
        if "optimizer" in optimizer_config:
            ...
            for cm in self.instance.named_children():
                ...
                if child_module_name in optimizer_params_config.keys():
                    specific_config = split_parameters(
                        optimizer_params_config[child_module_name]
                    )
                    logging.info(
                        f'Custom config for {child_module_name}. Params: {str(specific_config["params"])}'
                    )
                    params = {
            ...
            self.optim_instance = optimizer(
                params=grouped_parameters, **optimizer_params_config["params"]
            )
            lrs = None
            ...
            if "lrs" in optimizer_params_config:
                ...
                self.lrs_instance = lrs(
                    self.optim_instance, **lrs_params_config["params"]
                )
            if self.lrs_instance is not None:
                self.instance.set_optimizers(
                    optimizer=self.optim_instance, lrs=self.lrs_instance
                )
            else:
                self.instance.set_optimizers(optimizer=self.optim_instance)


class DataModuleBuilder:
    def __init__(self, data_params: dict, do_val_init=True):
        datamodule_params = split_parameters(
            dict_to_config(data_params), ["datamodule"]
        )["datamodule"]
        datamodule_args = split_parameters(
            dict_to_config(datamodule_params), ["arguments"]
        )["arguments"]
        img_processing_args = split_parameters(
            dict_to_config(data_params), ["img_processing"]
        )["img_processing"]
            ...
            **{
                **{
                    **{
                        ...
                        "root_dir": datamodule_params["root_dirs"][
        ...
        active_learing_params = split_parameters(
            dict_to_config(data_params), ["active_learning"]
        )["active_learning"]
        ...
        if active_learing_params:
            ...
            labeler.label_samples(
                datamodule,
                samples_to_label,
                ranker.rank_samples_to_label(
                    datamodule.train_dataloader(get_labeled_share=False).dataset
                ),
            )
        ...
        logging.info(
            f"Length of labeled train dataset: {len(self.instance.labeled_train_dataset)}"
        )
        logging.info(
            f"Length of unlabeled train dataset: {len(self.instance.unlabeled_train_dataset)}"
        )
        if do_val_init:
            logging.info(
                f"Length of validation dataset: {len(self.instance.val_dataset)}"
            )
...
class TrainerBuilder:
    def __init__(
        ...
        result_dir: str,
        run_name: str,
        data: DataModuleBuilder = None,
    ):
        callback_args = split_parameters(dict_to_config(train_params), ["callbacks"])[
            "callbacks"
        ]
        callback_args["experiment_dir"] = result_dir
        trainer_args = split_parameters(dict_to_config(train_params), ["trainer"])[
            "trainer"
        ]
