...
class PSPNet(BaseNet):
    def __init__(self, backbone, nclass, args=None):
        super(PSPNet, self).__init__(backbone, nclass, args)
...
class PyramidPooling(pl.LightningModule):
    ...
    def forward(self, x):
        h, w = x.shape[-2:]
        feat1 = F.interpolate(
            self.conv1(self.pool1(x)), (h, w), mode="bilinear", align_corners=True
        )
        feat2 = F.interpolate(
            self.conv2(self.pool2(x)), (h, w), mode="bilinear", align_corners=True
        )
        feat3 = F.interpolate(
            self.conv3(self.pool3(x)), (h, w), mode="bilinear", align_corners=True
        )
        feat4 = F.interpolate(
            self.conv4(self.pool4(x)), (h, w), mode="bilinear", align_corners=True
        )
